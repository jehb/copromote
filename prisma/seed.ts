...
async function main() {
    ...
    try {
        ...
        const contactBrenda = await prisma.contact.create({
            data: { firstName: 'Brenda', lastName: 'Camp', company: 'Cooperative Market', jobTitle: 'Community Outreach', email: 'brenda.c@example.com', notes: 'Key contact.', type: 'Internal' }
        })
        const contactRuffin = await prisma.contact.create({
            data: { firstName: 'Ruffin', lastName: 'Slater', company: 'Cooperative Market', jobTitle: 'General Manager', email: 'ruffin@example.com', type: 'Internal' }
        })
