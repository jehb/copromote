...
**UI/UX & User Flow:**
- **External User Flow:** The non-profit visits `coopmarket.example.com/host-an-event`. They fill out a multi-step React Hook Form (using Zod validation) detailing their mission, requested dates, and required table space.
...
**Benefits:** Gamifies the co-op experience, encouraging members to shift more of their grocery budget to the cooperative.
**Deep Dive integration & Architecture:**
...
**Description:** Monitor local community Facebook groups and Nextdoor for mentions of the cooperative.
...
**Deep Dive integration & Architecture:**
- **Instagram Graph API:** Register a Meta App to subscribe to the `mentions` webhook. When the co-op account is tagged, the webhook delivers the post ID to the platform.
...
**Deep Dive integration & Architecture:**
- **Magic Link Auth:** External vendors receive a secure, expiring JWT link to view a specific `Project` (e.g., `coopmarket.example.com/guest/p/123xyz`).
