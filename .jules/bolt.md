...
## 2026-05-25 - Optimize Prisma N+1 queries with batched findMany and Set\n**Learning:** When dealing with loops that execute individual `findFirst` queries (causing N+1 performance bottlenecks), batch the queries using `findMany` with an `in` operator outside the loop.\n**Action:** Extract all required identifiers, perform a single `findMany` query, and map the results into a `Set` for O(1) lookups during the main loop.
